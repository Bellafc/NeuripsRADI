...
from FADI.vh_mdp.vh_graph.envs import belief, vh_env
sys.path.append(os.path.abspath(os.path.join(os.path.dirname(__file__), '../../..')))
from FADI.virtualhome.simulation.unity_simulator import comm_unity as comm_unity
...
class UnityEnvironment(BaseEnvironment):
    def __init__(self,
        ...
        #     self.comm = comm_unity.UnityCommunication(port=str(self.base_port), **executable_args)
        #     #self.comm = comm_unity.UnityCommunication(no_graphics=False, port="8080")
    ...
    def step(self, action_dict, ignore_walk=None, logging=None):
        ...
        if num_actions!=0:
            if self.recording:
                ...
                success, message = self.comm.render_script(script_lists,
                                                           ...
                                                           processing_time_limit=180,
                                                           skip_animation=True)
            # print('[INFO] failed reason: ', message)
            if not success:
        ...
        ## check edges
        for agent_i, script_list in enumerate(script_lists):
            correct_graph_flag, next_obs[agent_i] = self.check_edge(script_list, next_obs[agent_i], agent_i, fix_edge=False)
